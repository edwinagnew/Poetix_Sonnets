...
class PriorSampling(DecodeStrategy):
    ...
    def advance(self, log_probs, attn, prior=None, pos_prior=None, verbose=False):
        """Select next tokens randomly from the top k possible next tokens.
        ...
        if pos_prior is not None:
            #potentially play around with entropy threshold
            if verbose: print("changed the logs", end=" ")
            for i, bat in enumerate(pos_prior):
                if bat.sum() > 0: #or some parameter
                    log_probs[i][bat == 0] = -1e20
